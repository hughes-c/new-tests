...
numOps = 0        #0 600k; 1 10M; 2 16M

if (numOps == 0):
   #600k
   stats_out = "test-out-600k-c.csv"
   memOps = 600000
elif(numOps == 1):
   #10M
   stats_out = "test-out-10M-c.csv"
   memOps = 10000000
else:
   #16M
   stats_out = "test-out-16M-c.csv"
   memOps = 16000000
...
print "Creating system with an address space of " + str(hex(0)) + " to " + str(hex(memory_capacity_inB))
...
for next_group_id in range(hbmStacks):
   ...
   for sub_group_id in range(sub_mems):
      ...
      if backend == "simple":
         # Create DDR (Simple)
         print "Configuring DDR-Simple mem part %d..."%(next_mem + 1)
         ...
         # Create CramSim HBM
         print "Creating HBM controller " + str(next_mem + 1) + " out of " + str(hbmStacks) + "..."

         print " - Capacity: " + str(memory_capacity_inB / hbmStacks) + " per HBM"
